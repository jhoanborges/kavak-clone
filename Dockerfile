...
# -- base ---------------------------------------------------------
# node:22-alpine: Next 16 requires Node >= 20.9. npm ships with the node image.
...
# `npm ci` installs exactly what package-lock.json pins and fails if it is out of
# sync (instead of silently mutating it) — the reproducible-build install.
RUN npm ci
