 ...
 * pide /api/imagen/<nombre> y el servidor las trae de {CATALOGO}/thumbnail.
 *
...
export async function GET(
  ...
  if (!CATALOGO_ORIGIN) {
    return NextResponse.json(
      { error: "CATALOGO_URL no está configurado en el servidor." },
      { status: 500 }
    );
  }

  try {
    const res = await fetch(`${CATALOGO_ORIGIN}/thumbnail/${nombre}`, {
      next: { revalidate },
